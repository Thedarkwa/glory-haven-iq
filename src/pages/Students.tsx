...
import { useData, classCategories, classNames } from "@/contexts/DataContext";
...
export default function Students() {
  ...
  const filtered = students
    ...
    .filter(s => s.full_name.toLowerCase().includes(search.toLowerCase()) || s.class.toLowerCase().includes(search.toLowerCase()) || s.student_id.toLowerCase().includes(search.toLowerCase()));

  const openAdd = () => { setEditing(null); setForm(emptyForm); setDialogOpen(true); };
  const openEdit = (s: Student) => { setEditing(s); setForm({ full_name: s.full_name, date_of_birth: s.date_of_birth || "", gender: s.gender, class: s.class, guardian: s.guardian || "", contact: s.contact || "" }); setDialogOpen(true); };

  const handleSave = async () => {
    if (!form.full_name.trim()) { toast.error("Full name is required"); return; }
    if (editing) { await updateStudent(editing.id, form); toast.success("Student updated"); }
    else { await addStudent(form); toast.success("Student added"); }
    setDialogOpen(false);
  };

  const handleDelete = async () => {
    if (editing) { await deleteStudent(editing.id); toast.success("Student deleted"); }
    ...
    <div>
      ...
      <Tabs value={tab} onValueChange={setTab} className="mb-4">
        <TabsList className="flex flex-wrap h-auto gap-1">
          {classCategories.map(c => <TabsTrigger key={c.label} value={c.label} className="text-xs">{c.label}</TabsTrigger>)}
        </TabsList>
      </Tabs>
      ...
      <Dialog open={dialogOpen} onOpenChange={setDialogOpen}>
        <DialogContent className="sm:max-w-md">
          ...
          <div className="grid gap-4 py-4">
            ...
            <div className="grid grid-cols-2 gap-4">
              <div className="grid gap-2"><Label>Guardian</Label><Input value={form.guardian} onChange={e => setForm({ ...form, guardian: e.target.value })} /></div>
              <div className="grid gap-2"><Label>Contact</Label><Input value={form.contact} onChange={e => setForm({ ...form, contact: e.target.value })} /></div>
            </div>
          </div>
          <DialogFooter>
      ...
      <DeleteDialog open={deleteOpen} onClose={() => setDeleteOpen(false)} onConfirm={handleDelete} title="Delete Student" description={`Delete ${editing?.full_name}?`} />
    </div>
  );
}
